...
def get_feature_representations(model, args: dict) -> (list, list):
    ...
    content_images = load_and_process_images_for_model(args["content_paths"], args)
    style_images = load_and_process_images_for_model(args["style_paths"], args)
...
def compute_loss(
    ...
    denoise_score += get_denoise_loss(init_image, args)
...
def compute_grads(cfg: dict, args: dict):
    ...
    with tf.GradientTape() as tape:
        all_loss = compute_loss(args, **cfg)
...
def run_style_transfer(args: dict) -> (np.ndarray, float):
    ...
    styles_features, contents_features = get_feature_representations(model, args)
    ...
    if args["use_first_image"]:
        init_image = load_and_process_images_for_model(
    ...
    for i in range(args["num_iterations"]):
        # Compute gradients
        grads, all_loss = compute_grads(cfg, args)
...
if __name__ == "__main__":
    ...
    best_img.save(os.path.join(args["saving_folder"], args["best_image_name"]))
